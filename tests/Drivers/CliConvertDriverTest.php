...
class CliConvertDriverTest extends TestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        if ($return !== 0)
            throw new RuntimeException('ImageMagick convert not available');
        // set up and clear out output directory
        static::$output = __DIR__ . '/../output/CliConvertDriverTest';
        if (!is_dir(static::$output))
            mkdir(static::$output);
        foreach (glob(static::$output . '/*') as $file)
            unlink($file);
        ...
        if (!extension_loaded('gd'))
            throw new RuntimeException('GD required to generate test fixtures');
    ...
    protected function setUp(): void
    {
        ...
        SmolImage::setDriver(new CliConvertDriver());
    }
    ...
    {
        $info = getimagesize('data://application/octet-stream;base64,' . base64_encode($this->image(static::$square)->string()));
    ...
    {
        $info = getimagesize('data://application/octet-stream;base64,' . base64_encode($this->image(static::$landscape)->string()));
        $this->assertEquals(200, $info[0]);
    ...
    {
        $info = getimagesize('data://application/octet-stream;base64,' . base64_encode($this->image(static::$portrait)->string()));
    ...
    public function test_throws_with_invalid_executable(): void
    {
        SmolImage::setDriver(new CliConvertDriver('/nonexistent/convert'));
        $this->expectException(RuntimeException::class);
        $this->image(static::$square)->save($this->outputPath('should_fail.png'));
    }
